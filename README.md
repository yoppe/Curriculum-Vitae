...
Ruby|６ヶ月／新卒時代、仕事でもプライベートでもある程度使っていた言語。今も書けなくはないが大抵のものがScalaとPythonでこと足りるため、最近は使っていない
...
Ruby on Rails|４ヶ月／チュートリアルを行い、既存プロジェクトの追加開発時に少し触った程度
...
クラウドサービス|４年／AWS(EC2,VPC,S3,RDS,etc.)/GCP(GCE,GKE,etc.)の経験があり、AWSの小規模なサーバ運用環境であれば仕事で単独での構築経験あり
インフラ自動化(Terraform)|１年／AWS,GCPともに経験あり。AWSは構築経験あるが、GCPは保守のみ経験
インフラ自動化(Ansible/Chef)|１年／Chefは保守経験のみ。最近はAnsibleばかりで、Ansibleは構築経験あり
Vagrant|６ヶ月／[ネットワーク周りの記事](http://labs.septeni.co.jp/entry/20140707/1404670069)を書ける程度には理解
Docker|１年／仕組みを理解し、簡単な設定をいじれる程度に経験あり
Kubernetes|６ヶ月／構築経験はないが、仕事で保守経験あり。簡単なトラブルシューティングができるレベルで習得
